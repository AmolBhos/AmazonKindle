...
private By FindQuantitySelection = By.cssSelector("option[value=\"2\"]");
...
public KindleOrderPage(WebDriver driver) {
	...
	//ScriptFactory.getTest().log(LogStatus.PASS, "verifyHomePage : User has navigated to TATA Home Page");
}
...
public boolean verifyQuantitySelection() throws InterruptedException {
	boolean res = false;	
	 boolean b = driver.findElement(FindQuantitySelection).isDisplayed();
	if(b==true){
		res=true;
	}
